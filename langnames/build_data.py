...
from langnames import normalize_name
from langnames.util import data_filename
from langnames.language_lists import CLDR_LANGUAGES
from langnames.registry_parser import parse_registry
...
def read_cldr_names(language, category):
    ...
    """
    filename = data_filename('cldr-localenames-modern/main/{}/{}.json'.format(language, category))
...
def get_name_languages():
    cldr_main_path = Path(data_filename("cldr-localenames-modern/main"))
    languages = [
...
DISPLAY_SEPARATORS_BY_LANGUAGE = {
    'und': ', ',
    'am': '፣',
    'ar': '، ',
    'brx': ',',
...
def build_data():
    ...
    with open('data_dicts.py', 'w', encoding='utf-8') as outfile:
        print(GENERATED_HEADER, file=outfile)
        write_python_dict(outfile, 'DISPLAY_SEPARATORS', DISPLAY_SEPARATORS_BY_LANGUAGE)
